        ...
        try:
            ...
            for msg_id in list(all_ids)[:100]:  # cap at 100 per folder
                try:
                    ...
                    if fetch_status != "OK" or not msg_data or not msg_data[0] or not msg_data[0][1]:
                        continue
                    raw = msg_data[0][1]
...
def classify_email_with_openai(api_key: str, sender: str, subject: str, body: str) -> dict:
    """
    Use OpenAI to classify a Gmail notification email as:
...
def run_inbox_monitor(config: dict, db_conn) -> dict:
    ...
    if not all([gmail_address, app_password, openai_api_key]):
        return {"error": "Gmail or OpenAI credentials not configured"}
    ...
    for item in emails:
        try:
            ...
            classification = classify_email_with_openai(openai_api_key, sender, subject, body)
