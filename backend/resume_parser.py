...
def parse_resume_with_openai(api_key: str, resume_text: str) -> dict:
    """
    Use OpenAI to parse raw resume text into structured data.
...
def parse_resume(pdf_path: str, openai_api_key: str) -> dict:
    """
    Full pipeline: extract text from PDF → parse with OpenAI → return structured dict.
    ...
    structured = parse_resume_with_openai(openai_api_key, resume_text)
